...
static int LookupLetter2(Translator *tr, unsigned int letter, char *ph_buf, size_t ph_size)
{
	int len;
	...
	if (Lookup(tr, &single_letter[1], ph_buf, ph_size) == 0) {
		single_letter[1] = ' ';
		if (Lookup(tr, &single_letter[2], ph_buf, ph_size) == 0)
			TranslateRules(tr, &single_letter[2], ph_buf, ph_size, NULL, 0, NULL);
	}
	return ph_buf[0];
}
...
void LookupAccentedLetter(Translator *tr, unsigned int letter, char *ph_buf, size_t ph_size)
{
	...
	if (accent_data != 0) {
		...
		if ((flags1 = Lookup(tr, accents_tab[accent1].name, ph_accent1, sizeof(ph_accent1))) != 0) {
			if (LookupLetter2(tr, basic_letter, ph_letter1, sizeof(ph_letter1)) != 0) {
				if (accent2 != 0) {
					flags2 = Lookup(tr, accents_tab[accent2].name, ph_accent2, sizeof(ph_accent2));
					if (flags2 & FLAG_ACCENT_BEFORE) {
						strcpy(ph_buf, ph_accent2);
						ph_size -= strlen(ph_buf);
				...
				if (letter2 != 0) {
					// ligature
					LookupLetter2(tr, letter2, ph_letter2, sizeof(ph_letter2));
					snprintf(ph_buf, ph_size, "%s%c%s%c%s%s", ph_accent1, phonPAUSE_VSHORT, ph_letter1, phonSTRESS_P, ph_letter2, ph_accent2);
				} else {
					if (accent1 == 0)
						strcpy(ph_buf, ph_letter1);
					else if ((tr->langopts.accents & 1) || (flags1 & FLAG_ACCENT_BEFORE) || (accents_tab[accent1].accent_flags & 1))
						snprintf(ph_buf, ph_size, "%s%c%c%s", ph_accent1, phonPAUSE_VSHORT, phonSTRESS_P, ph_letter1);
					else
						snprintf(ph_buf, ph_size, "%c%s%c%s%c", phonSTRESS_2, ph_letter1, phonPAUSE_VSHORT, ph_accent1, phonPAUSE_VSHORT);
				}
			}
		}
	}
...
void LookupLetter(Translator *tr, unsigned int letter, int next_byte, char *ph_buf1, size_t ph_size1, int control)
{
	...
	if (next_byte == -1) {
		...
		if (Lookup(tr, &single_letter[2], ph_buf1, ph_size1) != 0)
			return;

		single_letter[1] = '_';
		if (Lookup(tr, &single_letter[1], ph_buf3, sizeof(ph_buf3)) != 0)
		...
		if (Lookup(translator3, &single_letter[2], ph_buf3, sizeof(ph_buf3)) != 0) {
			// yes, switch to English and re-translate the word
			snprintf(ph_buf1, ph_size1, "%c", phonSWITCH);
		}
	...
	if ((letter <= 32) || iswspace(letter)) {
		...
		Lookup(tr, &single_letter[1], ph_buf1, ph_size1);
		return;
	}
	...
	if (Lookup(tr, &single_letter[1], ph_buf3, sizeof(ph_buf3)) == 0) {
		single_letter[1] = ' ';
		if (Lookup(tr, &single_letter[2], ph_buf3, sizeof(ph_buf3)) == 0)
			TranslateRules(tr, &single_letter[2], ph_buf3, sizeof(ph_buf3), NULL, FLAG_NO_TRACE, NULL);
	}

	if (ph_buf3[0] == 0)
		LookupAccentedLetter(tr, letter, ph_buf3, sizeof(ph_buf3));
...
int TranslateRoman(Translator *tr, char *word, char *ph_out, size_t ph_size, WORD_TAB *wtab)
{
	...
	Lookup(tr, "_roman", ph_roman, sizeof(ph_roman)); // precede by "roman" if _rom is defined in *_list
	p = &ph_out[0];
	...
	TranslateNumber(tr, &number_chars[2], p, ph_size - (p - ph_out), flags, wtab, num_control);
...
static int LookupThousands(Translator *tr, int value, int thousandplex, int thousands_exact, char *ph_out, size_t ph_size)
{
	...
	if (value > 0) {
		if (thousands_exact & 1) {
			if (thousands_exact & 2) {
				...
				found_value = Lookup(tr, string, ph_thousands, sizeof(ph_thousands));
			}
	...
	if (found_value == 0) {
		if ((value % 100) >= 20)
			Lookup(tr, "_0of", ph_of, sizeof(ph_of));
		...
		if (thousands_exact & 1) {
			if (thousands_exact & 2) {
				...
				found = Lookup(tr, string, ph_thousands, sizeof(ph_thousands));
			}
		...
		if (found == 0) {
			...
			if (Lookup(tr, string, ph_thousands, sizeof(ph_thousands)) == 0) {
				if (thousandplex > 3) {
					snprintf(string, sizeof(string), "_0M%d", thousandplex-1);
					if (Lookup(tr, string, ph_buf, sizeof(ph_buf)) == 0) {
						// say "millions" if this name is not available and neither is the next lower
						Lookup(tr, "_0M2", ph_thousands, sizeof(ph_thousands));
				...
				if (ph_thousands[0] == 0) {
					...
					if ((found_value = Lookup(tr, string, ph_thousands, sizeof(ph_thousands))) == 0)
						Lookup(tr, "_0M1", ph_thousands, sizeof(ph_thousands));
					speak_missing_thousands = 2;
				}
			}
		}
	}
	snprintf(ph_out, ph_size, "%s%s", ph_of, ph_thousands);
...
static int LookupNum2(Translator *tr, int value, int thousandplex, const int control, char *ph_out, size_t ph_size)
{
	...
	if ((control & 2) && (n_digit_lookup == 2)) {
		...
		if (digit_lookup[0] == 0) {
			...
			if (control & 8) {
				...
				if ((found = Lookup(tr, string, ph_digits, sizeof(ph_digits))) == 0) {
					snprintf(string, sizeof(string), "_%df", value);
					found = Lookup(tr, string, ph_digits, sizeof(ph_digits));
				}
			} else if (is_ordinal) {
		...
		if ((value < 10) && (control & 0x10)) {
			// speak leading zero
			Lookup(tr, "_0", ph_tens, sizeof(ph_tens));
		} else {
			if (found)
				ph_tens[0] = 0;
			else {
				if (is_ordinal) {
					...
					if (Lookup(tr, string, ph_tens, sizeof(ph_tens)) != 0) {
						found_ordinal = 1;
				...
				if (found_ordinal == 0) {
					...
					Lookup(tr, string, ph_tens, sizeof(ph_tens));
				}
				...
				if (units > 0) {
					...
					if ((control & 2) && (digit_lookup[0] != 0)) {
						...
						if (found == 0) {
							...
							Lookup(tr, string, ph_digits, sizeof(ph_digits));
						}
					}
				}
			}
		...
		if ((is_ordinal) && (found_ordinal == 0) && (ph_ordinal[0] == 0)) {
			if ((value >= 20) && (((value % 10) == 0) || (tr->langopts.numbers & NUM_SWAP_TENS)))
				Lookup(tr, "_ord20", ph_ordinal, sizeof(ph_ordinal));
			if (ph_ordinal[0] == 0)
				Lookup(tr, "_ord", ph_ordinal, sizeof(ph_ordinal));
		}

		if ((tr->langopts.numbers & (NUM_SWAP_TENS | NUM_AND_UNITS)) && (ph_tens[0] != 0) && (ph_digits[0] != 0)) {
			Lookup(tr, "_0and", ph_and, sizeof(ph_and));

			if ((is_ordinal) && (tr->langopts.numbers2 & NUM2_ORDINAL_NO_AND))
				ph_and[0] = 0;

			if (tr->langopts.numbers & NUM_SWAP_TENS)
				snprintf(ph_out, ph_size, "%s%s%s%s", ph_digits, ph_and, ph_tens, ph_ordinal);
			else
				snprintf(ph_out, ph_size, "%s%s%s%s", ph_tens, ph_and, ph_digits, ph_ordinal);
			used_and = 1;
		} else {
			...
			if ((tr->langopts.numbers2 & NUM2_ORDINAL_DROP_VOWEL) && (ph_ordinal[0] != 0)) {
				ix = snprintf(ph_out, ph_size, "%s%s", ph_tens, ph_digits);
				if ((ix > 0) && (phoneme_tab[(unsigned char)(ph_out[ix-1])]->type == phVOWEL))
					ix--;
				snprintf(&ph_out[ix], ph_size-ix, "%s", ph_ordinal);
			} else
				snprintf(ph_out, ph_size, "%s%s%s", ph_tens, ph_digits, ph_ordinal);
		}
	}
...
static int LookupNum3(Translator *tr, int value, char *ph_out, size_t ph_size, bool suppress_null, int thousandplex, int control)
{
	...
	if ((hundreds > 0) || say_zero_hundred) {
		...
		if (ordinal && (tensunits == 0)) {
			...
			found = Lookup(tr, "_0Co", ph_100, sizeof(ph_100));
		}
		if (found == 0) {
			if (tensunits == 0) {
				// special form for exact hundreds?
				found = Lookup(tr, "_0C0", ph_100, sizeof(ph_100));
			}
			if (!found)
				Lookup(tr, "_0C", ph_100, sizeof(ph_100));
		}
		...
		if ((hundreds > 0) || say_zero_hundred) {
			if ((tr->langopts.numbers & NUM_AND_HUNDRED) && ((control & 1) || (ph_thousands[0] != 0)))
				Lookup(tr, "_0and", ph_thousand_and, sizeof(ph_thousand_and));
			...
			if ((hundreds == 0) && say_zero_hundred)
				Lookup(tr, "_0", ph_digits, sizeof(ph_digits));
			else {
				if ((hundreds == 1) && (tr->langopts.numbers2 & NUM2_OMIT_1_HUNDRED_ONLY) && ((control & 1) == 0)) {
					...
					if (!found) {
						...
						found = Lookup(tr, string, ph_digits, sizeof(ph_digits));  // is there a specific pronunciation for n-hundred ?
					}
				}
		...
		if ((control & 2) && (tr->langopts.numbers2 & NUM2_MULTIPLE_ORDINAL)) {
			...
			if ((value > 100) || ((control & 1) && (thousandplex == 0))) {
				if ((tr->langopts.numbers & NUM_HUNDRED_AND) || ((tr->langopts.numbers & NUM_HUNDRED_AND_DIGIT) && (tensunits < 10)))
					Lookup(tr, "_0and", ph_hundred_and, sizeof(ph_hundred_and));
			}
	...
	snprintf(ph_out, ph_size, "%s%s%c%s", buf1, ph_hundred_and, phonEND_WORD, buf2);
...
static int TranslateNumber_1(Translator *tr, char *word, char *ph_out, size_t ph_size, unsigned int *flags, WORD_TAB *wtab, int control)
{
	...
	*flags = 0;
		...
		*p = 0;
		...
		if (suffix[0] != 0) {
			if ((tr->langopts.ordinal_indicator != NULL) && (strcmp(suffix, tr->langopts.ordinal_indicator) == 0))
				...
				if (Lookup(tr, string, ph_ordinal2, sizeof(ph_ordinal2))) {
					// this is an ordinal suffix
					ordinal = 2;
					flags[0] |= FLAG_SKIPWORDS;
					skipwords = 1;
					snprintf(string, sizeof(string), "_x#%s", suffix);
					Lookup(tr, string, ph_ordinal2x, sizeof(ph_ordinal2x)); // is there an alternate pronunciation?
				}
			}
		}
	}
	...
	if ((word[0] == '0') && (prev_thousands == 0) && (word[1] != ' ') && (word[1] != tr->langopts.decimal_sep)) {
		if ((n_digits == 2) && (word[3] == ':') && IsDigit09(word[5]) && isspace(word[7])) {
			...
			for (ix = 0; (word[ix] == '0') && (ix < (n_digits-1)); ix++)
				Lookup(tr, "_0", &ph_zeros[strlen(ph_zeros)], sizeof(ph_zeros)-strlen(ph_zeros));
		}
	}
	...
	if ((word[n_digits] == tr->langopts.decimal_sep) && IsDigit09(word[n_digits+1])) {
		...
		Lookup(tr, "_dpt", ph_append, sizeof(ph_append));
		decimal_point = 0x100;
		...
		snprintf(string, sizeof(string), "_0M%d", thousandplex+1);
		if (Lookup(tr, string, buf1, sizeof(buf1)) == 0) {
			snprintf(string, sizeof(string), "_0M%d", thousandplex);
			Lookup(tr, string, ph_append, sizeof(ph_append));
		}
	}

	if ((ph_append[0] == 0) && (word[n_digits] == '.') && (thousandplex == 0))
		Lookup(tr, "_.", ph_append, sizeof(ph_append));
	...
	if (thousandplex == 0) {
		...
		if (IsDigit09(p[-1])) {
			p2 = p - 1;
			if (LookupDictList(tr, &p2, buf_digit_lookup, sizeof(buf_digit_lookup), flags, FLAG_SUFX, wtab)) // lookup 2 digits
				n_digit_lookup = 2;
		}
		...
		if ((buf_digit_lookup[0] == 0) && (*p != '0')) {
			...
			if (LookupDictList(tr, &p, buf_digit_lookup, sizeof(buf_digit_lookup), flags, FLAG_SUFX, wtab)) // don't match '0', or entries with $only
				n_digit_lookup = 1;
		}
		...
		if (prev_thousands == 0) {
			if ((decimal_point == 0) && (ordinal == 0)) {
				...
				if (Lookup(tr, string, ph_out, ph_size))
					return 1;
			}
			...
			if (tr->langopts.numbers2 & NUM2_PERCENT_BEFORE) {
				...
				while ((*p2 != ' ') && (*p2 != 0))
					p2++;
				if (p2[1] == '%') {
					Lookup(tr, "%", ph_out, ph_size);
					ph_size -= strlen(ph_out);
	...
	LookupNum3(tr, value, ph_buf, sizeof(ph_buf), suppress_null, thousandplex, prev_thousands | ordinal | decimal_point);
	if ((thousandplex > 0) && (tr->langopts.numbers2 & NUM2_SWAP_THOUSANDS))
		snprintf(ph_out, ph_size, "%s%s%c%s%s", ph_zeros, ph_append, phonEND_WORD, ph_buf2, ph_buf);
	else
		snprintf(ph_out, ph_size, "%s%s%s%c%s", ph_zeros, ph_buf2, ph_buf, phonEND_WORD, ph_append);
	...
	while (decimal_point) {
		...
		case NUM_DFRACTION_2:
			...
			while (word[n_digits] == '0') {
				Lookup(tr, "_0", buf1, sizeof(buf1));
				strcat(ph_out, buf1);
				decimal_count--;
				n_digits++;
			}
			if ((decimal_count <= max_decimal_count) && IsDigit09(word[n_digits])) {
				LookupNum3(tr, atoi(&word[n_digits]), buf1, sizeof(buf1), false, 0, 0);
				strcat(ph_out, buf1);
		...
		case NUM_DFRACTION_6: // kazakh, always say "tenths" etc, before the decimal fraction
			LookupNum3(tr, atoi(&word[n_digits]), ph_buf, sizeof(ph_buf), false, 0, 0);
		...
		while (IsDigit09(c = word[n_digits]) && (strlen(ph_out) < (N_WORD_PHONEMES - 10))) {
			...
			len = strlen(ph_out);
			snprintf(&ph_out[len], ph_size-len, "%c%s", phonEND_WORD, buf1);
		}

		// something after the decimal part ?
		if (Lookup(tr, "_dpt2", buf1, sizeof(buf1)))
			strcat(ph_out, buf1);

		if ((c == tr->langopts.decimal_sep) && IsDigit09(word[n_digits+1])) {
			Lookup(tr, "_dpt", buf1, sizeof(buf1));
			strcat(ph_out, buf1);
		} else
			decimal_point = 0;
	}
...
int TranslateNumber(Translator *tr, char *word1, char *ph_out, size_t ph_size, unsigned int *flags, WORD_TAB *wtab, int control)
{
	...
	if (tr->langopts.numbers != 0)
		return TranslateNumber_1(tr, word1, ph_out, ph_size, flags, wtab, control);
	return 0;
}
